...
"""
A module to provide extended functionality from the
...
def delete(session: Session, url: str, *args, **kwargs) -> Response:
	"""Sends a DELETE request to the API service. All extra arguments
	are passed to the session's request handler.

	:param session: The session of which to send the request from.
	...
	"""
	logger.debug(f"Sending DELETE request to {url} supplying " \
		f"{args} and {kwargs} as parameters.")

	return session.delete(url, *args, **kwargs)

def get(session: Session, url: str, *args, **kwargs) -> Response:
	"""Sends a GET request to the API service. All extra arguments
	are passed to the session's request handler.
	...
	:rtype: :class:`requests.models.Response`
	"""
	logger.debug(f"Sending GET request to {url} supplying " \
		f"{args} and {kwargs} as parameters.")

	return session.get(url, *args, **kwargs)

def patch(session: Session, url: str, *args, **kwargs) -> Response:
	"""Sends a PATCH request to the API service. All extra arguments
	are passed to the session's request handler.
	...
	:rtype: :class:`requests.models.Response`
	"""
	logger.debug(f"Sending PATCH request to {url} supplying " \
		f"{args} and {kwargs} as parameters.")

	return session.patch(url, *args, **kwargs)

def post(session: Session, url: str, *args, **kwargs) -> Response:
	"""Sends a POST request to the API service. All extra arguments
	are passed to the session's request handler.
	...
	:rtype: :class:`requests.models.Response`
	"""
	logger.debug(f"Sending POST request to {url} supplying " \
		f"{args} and {kwargs} as parameters.")

	return session.post(url, *args, **kwargs)

def put(session: Session, url: str, *args, **kwargs) -> Response:
	"""Sends a PUT request to the API service. All extra arguments
	are passed to the session's request handler.
	...
	:rtype: :class:`requests.models.Response`
	"""
	logger.debug(f"Sending PUT request to {url} supplying " \
		f"{args} and {kwargs} as parameters.")
...
class AttributeFinderMixin(list):
	...
	for attributes matching the value supplied.
	"""

	def first_matching_attribute(self, attribute: str, value: Any):
	...
	def iter_all_matching_attribute(self, attribute: str, value: Any):
		"""Locates all Attribute instances that match the attribute provided.
    ...
    def _retry_using_http_Negotiate_auth(self, response, scheme, args):
        ...
        if self._host is None:
            ...
            try:
                ...
                logger.info('Skipping canonicalization of name %s due to error: %s', self._host, e)
        ...
        try:
            ...
            logger.debug('Sending Initial Context Token - error={} authenticated={}'.format(error, clientauth.authenticated))
        except pywintypes.error as e:
            logger.debug('Error calling {}: {}'.format(e[1], e[2]), exc_info=e)
            return response
        ...
        if response2.status_code != 401:
            ...
            if final is not None:
                try:
                    ...
                    error, auth = clientauth.authorize(sec_buffer)
                    logger.debug('Kerberos Authentication succeeded - error={} authenticated={}'.format(error, clientauth.authenticated))
                except TypeError:
                    pass
        ...
        try:
            error, auth = clientauth.authorize(sec_buffer)
            request.headers['Authorization'] = '{} {}'.format(scheme, base64.b64encode(auth[0].Buffer).decode('ASCII'))
            logger.debug('Sending Response - error={} authenticated={}'.format(error, clientauth.authenticated))
        except pywintypes.error as e:
            logger.debug('Error calling {}: {}'.format(e[1], e[2]), exc_info=e)
            return response
